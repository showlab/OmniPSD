#!/bin/bash
# Train image-content foreground extraction (Flux-Kontext LoRA).
...
accelerate launch \
  ...
  train/train.py \
  --dataset_base_path /PATH/TO/DATASET/content \
  --dataset_metadata_path /PATH/TO/DATASET/content/front_metadata.csv \
  ...
  --output_path "/PATH/TO/OUTPUT/content_front" \
  --lora_base_model "dit" \
  ...
  --save_steps 5000 \
  --lora_checkpoint "/PATH/TO/PRETRAINED_LORA/content_front.safetensors"
